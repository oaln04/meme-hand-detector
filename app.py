...
class HandGestureDetector:
    ...
    def _classify_gesture(landmarks, handedness: str) -> str:
        ...
        if raised_fingers == 4 and thumb_is_open:
            return "five"

        return COUNT_GESTURES.get(raised_fingers, "four")
    ...
    def _finger_states(landmarks, handedness: str) -> Tuple[bool, bool, bool, bool, bool]:
        ...
        # thumb and only use it to separate four fingers from an open hand.
        thumb_tip = landmarks[4]
        thumb_ip = landmarks[3]
        thumb_mcp = landmarks[2]
        thumb_side_open = thumb_tip.x < thumb_ip.x if handedness == "Right" else thumb_tip.x > thumb_ip.x
        thumb_vertical_open = thumb_tip.y < thumb_ip.y < thumb_mcp.y
        thumb_is_open = thumb_side_open or thumb_vertical_open
    ...
    def _is_bite_gesture(landmarks, middle_up: bool, ring_up: bool, pinky_up: bool) -> bool:
        ...
        pinch_is_close = pinch_distance < max(0.045, palm_scale * 0.38)
        index_is_active = index_tip.y < index_mcp.y or index_pip.y < index_mcp.y
        other_fingers_down = not any((middle_up, ring_up, pinky_up))

        return pinch_is_close and index_is_active and other_fingers_down
    ...
    def _is_thumb_gesture(landmarks, index_up: bool, middle_up: bool, ring_up: bool, pinky_up: bool) -> bool:
        ...
        return thumb_points_up and other_fingers_down
...
def most_common_value(values: deque[str]) -> str:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--history", type=int, default=8, help="Frames used for smoothing gestures. Default: 8")
...
def main() -> None:
    ...
    gesture_history: deque[str] = deque(maxlen=max(args.history, 1))
    ...
    try:
        while True:
            ...
            gesture_history.append(gesture_value)
            stable_gesture = most_common_value(gesture_history)
